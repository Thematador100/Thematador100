            ...
            <div className="p-4 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-center">
                    <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-700">
                        ...
                        <p className="text-2xl font-bold text-green-400">{play.incomePotential}</p>
                    </div>
                    <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-700">
                        <p className="text-xs text-slate-400 font-semibold uppercase">Time to First Cash</p>
                        <p className="text-2xl font-bold text-amber-300">{play.timeToFirstCash}</p>
                    </div>
                </div>
                 <div>
                    <h4 className="font-semibold text-orange-300 text-sm mb-1">Gatekeeper Bypass Tactic</h4>
                    <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-700">
                ...
                <div>
                    <h4 className="font-semibold text-orange-300 text-sm mb-1">Required Assets</h4>
                    <div className="flex flex-wrap gap-2">
                 ...
                 <div className="pt-3 border-t border-slate-800">
                    <h4 className="font-semibold text-orange-300 text-sm mb-2">Execution Steps</h4>
...
const LoneWolfReportComponent: React.FC<LoneWolfReportProps> = ({ report, onDeployAgents }) => {
    return (
        <div className="max-w-4xl mx-auto bg-slate-800/70 rounded-xl border-2 border-orange-500/50 shadow-2xl p-6 sm:p-8 animate-fade-in">
            <div className="text-center">
                <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-amber-400">
                    🐺 Lone Wolf Protocol
                </h2>
                <p className="text-slate-400 mt-2 mb-8">High-leverage, instant-monetization plays for the solo operator.</p>
                <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                    <h3 className="text-lg font-semibold text-orange-300">Executive Summary</h3>
            ...
            <div className="mt-8 space-y-8">
                {report.loneWolfPlays.map((play, index) => (
                    <LoneWolfPlayCard key={index} play={play} onDeploy={(p) => onDeployAgents(p, 'Lone Wolf Play')} />
                ))}
            </div>
