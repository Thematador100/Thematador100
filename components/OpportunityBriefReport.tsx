...
import DecisionDecompositionChart from './DecisionDecompositionChart';
import AngleUpliftHeatmap from './AngleUpliftHeatmap';
        ...
        <div className="space-y-6">
            <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                 ...
                 <div className="w-full h-72 md:h-96">
                    {barData.length > 0 ? <DecisionDecompositionChart data={barData} /> : <div className="h-full grid place-items-center text-sm text-slate-500">No data available</div>}
                 </div>
            </div>
            <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                 <h4 className="font-bold text-amber-300 mb-2">Angle Uplift by Emotion</h4>
                 <div className="w-full h-72 md:h-96">
                     {heatData.length > 0 ? <AngleUpliftHeatmap data={heatData} /> : <div className="h-full grid place-items-center text-sm text-slate-500">No data available</div>}
                 </div>
            </div>
        </div>
    );
        ...
        <div className="pt-8 mt-8 border-t-2 border-dashed border-cyan-500/30">
            ...
            {!generatedCode && (
                <div className="text-center">
                    ...
                    >
                        Build "{brief.aiPoweredSolution.resultsInAdvanceMechanism.name}" Tool
                    </button>
...
const OpportunityBriefReport: React.FC<OpportunityBriefReportProps> = ({ brief, generatedCode, onGenerateCodeFromOpportunity, landingPageBlueprint, landingPageCode, onGenerateLandingPageBlueprint, onGenerateLandingPageCode, onGenerateAIVideoFoundry }) => {
    const { gauntletVerdict, starvingCrowd, aspirinProblem, aiPoweredSolution, fastestPathToCash, mentalModelApplicable, quantitativeModel, dataVisualizationSuite, urgencyTriggers, marketMindAnalysis, methodology, marketSizeEstimate, urgencyLevel, asymmetricJVProtocol, businessInABoxAngle } = brief;
    
    const urgencyColor = {
        'High': 'text-red-400',
        'Medium': 'text-yellow-400',
        'Low': 'text-green-400',
    }[urgencyLevel || ''] || 'text-slate-300';
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                        ...
                        <ul className="space-y-1.5">
                            <CheckListItem label="Is it an Aspirin?" checked={gauntletVerdict.checklist.isAspirin} />
                            <CheckListItem label="Is it a Starving Crowd?" checked={gauntletVerdict.checklist.isStarvingCrowd} />
                            <CheckListItem label="Is the Market Big Enough?" checked={gauntletVerdict.checklist.isBigEnough} />
                            <CheckListItem label="Is the Market Reachable?" checked={gauntletVerdict.checklist.isReachable} />
                            <CheckListItem label="Is the Need URGENT?" checked={gauntletVerdict.checklist.isUrgent} />
                            <CheckListItem label="Can Sell for a High Price?" checked={gauntletVerdict.checklist.canSellHighPrice} />
                            <CheckListItem label="Has a Back-End?" checked={gauntletVerdict.checklist.hasBackEnd} />
                            <CheckListItem label="Is it a Tollbooth?" checked={gauntletVerdict.checklist.isTollbooth} />
                            <CheckListItem label="Is it Unique (USP)?" checked={gauntletVerdict.checklist.isUnique} />
                            <CheckListItem label="Is the Market Growing?" checked={gauntletVerdict.checklist.isGrowing} />
                        </ul>
                    </div>

                    {/* Right Column: The Plan */}
                    <div className="lg:col-span-2 space-y-6">
                         {(marketSizeEstimate || urgencyLevel) && (
                            <div>
                                ...
                                <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {marketSizeEstimate && (
                                        <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700 text-center">
                                            ...
                                            <p className="text-lg font-bold text-amber-300">{marketSizeEstimate}</p>
                                        </div>
                                    )}
                                    {urgencyLevel && (
                                        <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700 text-center">
                                            <p className="text-xs text-slate-400 font-semibold uppercase">Urgency Level</p>
                                            <p className={`text-lg font-bold ${urgencyColor}`}>{urgencyLevel}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div>
                         ...
                         {dataVisualizationSuite && (
                            <div className="pt-6 border-t border-slate-700">
                                ...
                                <DataVisualizationSuiteReport suite={dataVisualizationSuite} />
                            </div>
                         )}

                         {marketMindAnalysis && <MarketMindAnalysisReport analysis={marketMindAnalysis} />}

                         {urgencyTriggers && urgencyTriggers.length > 0 && (
                            <div>
                                ...
                                <div className="bg-red-900/20 border border-red-500/50 p-3 rounded-lg mt-2 space-y-1">
                                    {urgencyTriggers.map((trigger, i) => <p key={i} className="text-sm text-red-200">&#9888; {trigger}</p>)}
                                </div>
                            </div>
                        )}
