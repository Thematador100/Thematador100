  ...
  return (
    <div id={`discovered-audience-card-${index}`} className={`bg-slate-800/70 rounded-xl border ${isActive ? 'border-teal-500' : 'border-slate-700'} shadow-2xl p-6 flex flex-col space-y-4 h-full transition-colors`}>
      <div className="flex-grow">
        <h3 className="text-xl font-bold text-teal-300">{audience.audienceName}</h3>
        <p className="text-slate-400 mt-2 text-sm">{audience.summary}</p>
      </div>
      ...
      <QuantitativeSnapshot marketSize={audience.marketSizeEstimate} urgency={audience.urgencyLevel} />
       ...
       <div className="pt-4 border-t border-slate-700">
        ...
        <pre className="text-teal-300 text-xs whitespace-pre-wrap font-mono bg-slate-950 p-2 rounded-md">
            <code>{audience.quantitativeModel.decisionScoreFormula}</code>
        </pre>
      </div>
      ...
      <div className="pt-4 border-t border-slate-700 space-y-4">
        <div>
            <h4 className="font-semibold text-blue-400 mb-2 text-sm">Demographics</h4>
             <ul className="text-xs text-slate-300 space-y-1">
                <li><strong>Age:</strong> {audience.demographics.ageRange}</li>
                <li><strong>Income:</strong> {audience.demographics.incomeLevel}</li>
                <li><strong>Locations:</strong> {audience.demographics.commonLocations.join(', ')}</li>
            </ul>
        </div>

        {isB2B && 'motivations' in psychographics && (psychographics as Psychographics).motivations.length > 0 && (
            <div>
                ...
                {motivationsExpanded && (
                    <div className="space-y-2 animate-fade-in">
                        {(psychographics as Psychographics).motivations.map((m, i) => (
                            <div key={i} className="bg-slate-900/50 p-2 rounded-lg border border-slate-700">
                                <p className="font-bold text-blue-400 text-xs">{m.driver}</p>
                                <p className="text-slate-400 text-xs mt-1">{m.description}</p>
                                <p className="text-slate-300 text-xs mt-2 pt-2 border-t border-slate-800">
        ...
        {isB2B && 'dataSignals' in psychographics && (
             <div>
                <h4 className="font-semibold text-blue-400 mb-2 text-sm">Actionable Data Signals</h4>
                 <ul className="list-disc list-inside text-xs text-slate-300 space-y-1">
                  {(psychographics as Psychographics).dataSignals.map(p => <li key={p}>{p}</li>)}
                </ul>
            </div>
        )}
        
        {!isB2B && 'mediaConsumptionHabits' in psychographics && (
             <div>
                <h4 className="font-semibold text-blue-400 mb-2 text-sm">Media Consumption</h4>
                 <ul className="list-disc list-inside text-xs text-slate-300 space-y-1">
                  {(psychographics as B2CPsychographics).mediaConsumptionHabits.map(p => <li key={p}>{p}</li>)}
                </ul>
            </div>
        )}
         ...
         <div>
            <h4 className="font-semibold text-blue-400 mb-2 text-sm">Buying Triggers & Intent Signals</h4>
             <div className="space-y-2">
                {('buyingTriggers' in psychographics ? (psychographics as Psychographics).buyingTriggers : (psychographics as B2CPsychographics).b2cBuyingTriggers).map((t, i) => (
                    <div key={i} className="bg-slate-900/50 p-2 rounded-lg border border-slate-700">
                       <p className="font-bold text-teal-400 text-xs">{t.trigger}</p>
