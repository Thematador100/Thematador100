                    ...
                    {projects.length === 0 ? (
                        ...
                        <ul className="space-y-3">
                            {projects.map(project => (
                                <li key={project.id} className="bg-slate-900/50 p-3 rounded-md border border-slate-700 flex justify-between items-center">
                                    <div>
                                        <p className="font-semibold text-slate-200">{project.name}</p>
                                        <p className="text-xs text-slate-400">{new Date(project.timestamp).toLocaleString()}</p>
                                    </div>
