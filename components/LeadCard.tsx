...
const LeadCard: React.FC<LeadCardProps> = ({ prospect, isSelected, onToggleSelect, filter, advancedFilter }) => {
  ...
  const getStatusBadge = () => {
    ...
    return (
      <div 
        className={`${baseClasses} ${statusClasses} ${prospect.validationRationale ? 'cursor-pointer' : ''}`}
        onClick={() => prospect.validationRationale && setShowRationale(prev => !prev)}
  ...
  return (
    <div className="relative bg-slate-800/70 rounded-xl border border-slate-700 shadow-2xl p-6 flex flex-col space-y-4 transform hover:scale-105 hover:border-blue-500 transition-all duration-300">
      <input
        type="checkbox"
        className="absolute top-4 left-4 h-5 w-5 rounded bg-slate-700 border-slate-500 text-indigo-500 focus:ring-indigo-600 cursor-pointer"
      ...
      {showRationale && prospect.validationRationale && (
        <div className="p-3 bg-slate-900/50 border border-slate-700 rounded-lg transition-all duration-300 ease-in-out">
      ...
      <div className="pt-2 flex-grow flex flex-col justify-end">
        <div>
            <p className="text-xs font-semibold text-slate-400 mb-2">Pre-built Search Queries:</p>
