...
const SectionDisplay: React.FC<{ section: LandingPageSection }> = ({ section }) => {
    ...
    const getIcon = (type: LandingPageSection['sectionType']) => {
        switch (type) {
            case 'hero': return '🎯';
            case 'problem': return '🔥';
            case 'solution': return '💡';
            case 'features': return '✨';
            case 'socialProof': return '🗣️';
            case 'callToAction': return '🚀';
            case 'faq': return '❓';
            default: return '📄';
        }
    };
        ...
        <div className="bg-slate-950/70 border border-slate-700 rounded-lg">
            ...
            >
                <h5 className="font-semibold text-slate-200 flex items-center">
                    <span className="text-lg mr-2">{getIcon(section.sectionType)}</span>
                    {section.sectionType.charAt(0).toUpperCase() + section.sectionType.slice(1).replace(/([A-Z])/g, ' $1')}
                </h5>
                <span className={`transform transition-transform ${isOpen ? 'rotate-180' : ''}`}>▼</span>
            </button>
            {isOpen && (
                <div className="p-3 border-t border-slate-700 space-y-2">
                    <p className="text-sm font-bold text-cyan-300">{section.headline}</p>
                    {section.subheadline && <p className="text-xs text-slate-300 italic">"{section.subheadline}"</p>}
                    <p className="text-xs text-slate-400">{section.body}</p>
...
const LandingPageArchitect: React.FC<LandingPageArchitectProps> = ({ brief, blueprint, code, onGenerateBlueprint, onGenerateCode }) => {
    ...
    const handleGenerateBlueprint = () => {
        ...
        The target audience is the '${brief.starvingCrowd.name}' who are dealing with the problem: '${brief.aspirinProblem.problem}'. 
        The solution is called '${brief.aiPoweredSolution.solutionName}' and it offers '${brief.aiPoweredSolution.irresistibleOffer}'. 
        The 'Results-in-Advance' hook is '${brief.aiPoweredSolution.resultsInAdvanceMechanism.name}: ${brief.aiPoweredSolution.resultsInAdvanceMechanism.description}'. 
